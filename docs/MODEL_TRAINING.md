...
```
where `model_name`: either of `wayformer`, `scenetokens_student_naive`, `scenetokens_student`, or `scenetokens_teacher`. The model name needs to be specified.
...
* `scenario`: either of `waymo` or `nuscenes`. This will simply set the scenario sequence partition. **Default** value is `waymo`.
* `paths`: either of `waymo ` (Experiment 1), `waymo_causal_labeled` (Experiment 2), or `waymo_causal_unlabeled` (Experiment 3). Each specifies the paths to the data. **Default** value is `waymo`. See this [doc](./EXPERIMENTS.md) for details on each type of experiment.
* `dataset`: either of `waymo` or `scenarionet `. This specifies the input data representation. **Default** value is `waymo`. See this [doc](./DATA_PREPARATION.md) for details on how to prepare the data according to each type of input representation.
...
```bash
uv run -m scenetokens.eval ckpt_name=[epoch_name]
```
