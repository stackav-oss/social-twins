...
- *SafeSceneTokens*: which builds from Wayformer and adds a scenario tokenization layer with safety-relevance awareness. To use it, run `train.py` with `model=safe_scenetokens`. *NOTE*: This model requires additional labels which can be generated using the [ScenarioCharacterization](https://github.com/navarrs/ScenarioCharacterization) tool. This is done by setting `autolabel_agents=true` in `configs/train.yaml` and using the categorization files found in [this]() folder. If you have already post-processed the training data make sure to also set `overwrite_cache=true`.
