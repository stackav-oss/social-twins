...
def get_infos_from_protos(data_path, output_path=None, num_shards = 10, num_workers=8):
    ...
    for i, (start, end) in enumerate(start_ends):
        ...
        func = partial(process_waymo_data_with_scenario_proto, output_path=shard_path)
...
def run(raw_data_path: str, proc_data_path: str, split: str, num_shards: int, num_workers: int = 8) -> None:
    ...
    sample_infos = get_infos_from_protos(
        data_path=split_raw_data_path, output_path=scenario_path, num_shards=num_shards, num_workers=num_workers
    )
