...
def compute_confusion_matrix(labels: torch.Tensor, predictions: torch.Tensor) -> tuple[torch.Tensor, ...]:
...
def compute_accuracy(labels: torch.Tensor, predictions: torch.Tensor) -> tuple[torch.Tensor, ...]:
    ...
    """
    true_positives, _, false_positives, false_negatives = compute_confusion_matrix(labels, predictions)
