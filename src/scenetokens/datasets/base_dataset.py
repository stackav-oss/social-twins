...
from scenetokens.utils.constants import BIG_EPSILON, DataSplits
...
class BaseDataset(Dataset, ABC):
    ...
    def __init__(self, config: DictConfig) -> None:
        ...
        if self.config.load_data:
            ...
            self.blacklist = []
            if self.config.select_samples:
                sample_selection_filepath = Path(self.config.selection_filepath)
    ...
    def load_data(self) -> None:
        ...
        for data_path, data_tag in zip(self.data_path, self.data_tag, strict=False):
            ...
            print(f"Loaded {len(file_list)} samples from {data_path}")
            self.data_loaded.update(file_list)

            if self.config.store_data_in_memory:
                print("Loading data into memory...")
                for data_filepath in file_list:
    ...
    def get_centered_agent_data(  # noqa: PLR0915
        ...
        agent_ids = np.pad(agent_ids, ((0, 0), (0, size_to_pad), (0, 0), (0, 0)))
    ...
    def characterize_scenario(self, output: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for out in output:
            ...
            pad = out["pad"].astype(int)
            causal_idxs[-pad:] = False
