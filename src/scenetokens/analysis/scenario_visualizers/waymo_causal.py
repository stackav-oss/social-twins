...
class WaymoCausalVisualizer(BaseVisualizer):
    ...
    def plot_tokenized(
        ...
        for n, agent_id in enumerate(modeled_agent_ids):
            idx = np.where(agent_ids == agent_id)[0].item()
            if idx == ego_index:
                continue
        ...
        for apos, alen, awid, ahead, amask, color, score in zipped:
