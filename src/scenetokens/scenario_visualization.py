...
def main(config: DictConfig) -> float | None:
    ...
    start = time()
    output_path = Path(f"{config.output_path}/{config.split}_scenarios/{config.analysis.tag}")
    ...
    # Creates a ditionary of scenarios following the format {scenario_id: path/to/scenario_id.pkl}
    ...
    # Plot scenarios
    visualizer = hydra.utils.instantiate(config.analysis.visualizer)
