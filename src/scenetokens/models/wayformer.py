...
class Wayformer(BaseModel):
    ...
    def forward(self, batch: dict) -> ModelOutput:
        ...
        Returns:
            prediction (dict): dictionary containing the predicted outputs
                predicted_probability (batch_size, modes): the predicted probability for each of the predicted modes.
                predicted_trajectory (batch_size, modes, future_len, 2): the (X, Y) predicted trajectories per mode per
                    element in the batch.
            loss (torch.tensor): the computed loss with the gradients.
        """
